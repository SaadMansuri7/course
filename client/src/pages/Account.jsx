...
const Account = () => {
    ...
    let completed = 0, total = 0, progress = 0
    ...
    useEffect(() => {
        async function fetchEnrollments() {
            ...
            if (userId) {
                ...
                console.log('enrollments : ', enrolledCourses)
            }
        }
        fetchEnrollments();
    }, []);
        ...
        <>
            ...
            <div className="min-h-screen py-10 px-4">
                <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-xl p-8">
                    ...
                    {enrolledCourses && enrolledCourses.map((enrollment, index) => {
                        return (
                            <div key={index} className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                                <div className="bg-purple-100 p-5 rounded-lg text-center">
                                    <p className="text-sm text-gray-600">Enrolled Courses</p>
                                    <h3 className="text-2xl font-bold text-purple-700">{enrollment.course.courseData.units.length}</h3>
                                </div>
                                <div className="bg-green-100 p-5 rounded-lg text-center">
                                    <p className="text-sm text-gray-600">Completed Courses</p>
                                    <h3 className="text-2xl font-bold text-green-700">{enrollment.completedCount}</h3>
                                </div>
                            </div>
                        )
                    })}
